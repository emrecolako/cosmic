...
import { loadReadingInput, computeProfile, ReadingInput, CalculatedProfile } from "@/lib/profile";
...
export default function ResultsPage() {
  ...
  useEffect(() => {
    if (!input) return;
    let cancelled = false;
    Promise.all([computeProfile(input), loadContent(locale)]).then(([computed, localizedContent]) => {
      if (cancelled) return;
      if (!computed) { router.replace("/"); return; }
      setProfile(computed);
  ...
  const startAnalysis = useCallback(async (currentInput: ReadingInput, currentProfile: CalculatedProfile) => {
    ...
    try {
      const response = await fetch("/api/generate-reading", {
        ...
        body: JSON.stringify({
          ...currentInput,
          lifeStage: currentInput.lifeStages.map((s) => en.lifeStages[s]).join(", "),
          locale,
      ...
      setAiText(accumulated);
      if (!parseAnalysis(accumulated).combinedAnalysis) throw new Error("Incomplete analysis");
      setAiStatus("done");
    } catch {
  ...
  const retry = () => { if (input && profile) startAnalysis(input, profile); };

  return (
    <main className="min-h-screen pt-10">
      <div className="mx-auto max-w-3xl px-4 sm:px-6 py-12">
        <div className="mb-12 font-mono text-xs tracking-wider animate-fade-in">
          <Link href="/" className="text-ink-muted hover:opacity-70 transition-opacity uppercase">[← {t.results.newReading}]</Link>
          <h1 className="text-2xl sm:text-3xl tracking-wider text-ink mt-6 uppercase">{t.results.title}</h1>
          {input && <p className="text-ink-muted mt-1 uppercase">{formatMessage(t.results.forPerson, { name: input.fullName })}</p>}
        </div>

        {(!profile || !content) && (
          <div className="space-y-8">
            <Skeleton className="h-24 rounded-lg" />
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3"><StatCardSkeleton /><StatCardSkeleton /><StatCardSkeleton /><StatCardSkeleton /></div>
          </div>
        )}

        {profile && content && <CosmicProfile profile={profile} content={content} ai={ai} aiStatus={aiStatus} onRetry={retry} />}
      </div>
    </main>
  );
}
