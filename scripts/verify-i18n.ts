...
async function main(): Promise<void> {
  ...
  for (const locale of SUPPORTED_LOCALES) {
    ...
    assert(
      `catalog wizard reveal ${locale}`,
      messages.wizard.reveal.trim().length > 0
    );
  }
  ...
  const categories: NumerologyCategory[] = [
    "lifePath",
    "expression",
    "soulUrge",
    "personality",
  ...
  for (const locale of SUPPORTED_LOCALES) {
    ...
    for (const sign of signs) {
      assert(
        `content sign name ${locale}/${sign}`,
        content.signNames[sign].trim().length > 0
      );
      const zodiac = content.zodiac[sign];
      assert(
        `content zodiac ${locale}/${sign}`,
        zodiac.description.trim().length > 0 && zodiac.traits.length > 0
      );
    }
    for (const animal of animals) {
      assert(
        `content animal name ${locale}/${animal}`,
        content.animalNames[animal].trim().length > 0
      );
      assert(
        `content animal prose ${locale}/${animal}`,
        content.chineseAnimals[animal].trim().length > 0
      );
    }
    for (const category of categories) {
      for (const number of numbers) {
        ...
        assert(
          `content numerology ${locale}/${category}/${number}`,
  ...
  for (const locale of SUPPORTED_LOCALES) {
    ...
    assert(
      ...
      prompt.includes(LANGUAGE_NAMES[locale])
    );
    assert(
      `prompt marker snapshot ${locale}`,
      prompt.includes("<<<SNAPSHOT>>>")
    );
    assert(
      `prompt marker reading ${locale}`,
      prompt.includes("<<<READING>>>")
    );
    assert(
      `prompt marker season ${locale}`,
      prompt.includes("<<<SEASON>>>")
    );
    assert(
      `prompt marker toolkit ${locale}`,
      prompt.includes("<<<TOOLKIT>>>")
    );
    assert(
      `prompt English enum sun ${locale}`,
      prompt.includes(`Sun Sign: ${baseProfile.westernAstro.sunSign.sign}`)
    );
    assert(
      `prompt English enum animal ${locale}`,
      prompt.includes(`Animal: ${baseProfile.chineseZodiac.animal}`)
    );
    assert(
      `prompt English life stage ${locale}`,
      prompt.includes("Life Stage: Building career, Parent")
    );
  }
